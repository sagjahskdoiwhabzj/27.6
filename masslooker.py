...
def check_bot_running(user_id: int) -> bool:
    ...
    try:
        # ИСПРАВЛЕНИЕ: используем прямую проверку из БД вместо кеша
...
async def masslooking_worker(user_id: int):
    ...
    while user_masslooking_active.get(user_id, False):
        try:
            ...
            # ИСПРАВЛЕНИЕ: используем реальную логику обработки каналов
            try:
                # Устанавливаем глобальный shared_client для совместимости с существующими функциями
                ...
                if queue:
                    ...
                    for username in list(queue.keys()):
                        ...
                        try:
                            # ИСПРАВЛЕНИЕ: используем существующую функцию обработки поста
                            success = await process_single_post_from_channel(username)
                            if success:
                                ...
                                if channel_data['posts_processed'] >= channel_data['total_posts']:
                                    ...
                                    await finalize_channel_processing(username)
                                    user_processed_channels[user_id].add(username)
