...
async def load_bot_state():
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: восстанавливаем кеш сессий всех пользователей из БД
        try:
            # Получаем все пользовательские сессии из БД для восстановления кеша
            import aiosqlite
                    ...
                    for row in rows:
                        ...
                        if session_json:
                            try:
                                ...
                                logger.info(f"Восстановлена сессия пользователя {user_id} в кеше")
                            except Exception as e:
                                logger.warning(f"Ошибка восстановления сессии пользователя {user_id}: {e}")
        except Exception as e:
            logger.error(f"Ошибка восстановления кеша сессий: {e}")
        
        logger.info(f"Состояние бота загружено из базы данных. Восстановлено {len(user_sessions_cache)} сессий в кеше")
    except Exception as e:
